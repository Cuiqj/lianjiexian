...
- (void)viewDidLoad
{
    ...
    [self.uiBtnPromulgate setHidden:YES];
    [super viewDidLoad];
...
- (void)viewWillDisappear:(BOOL)animated{
    ...
    if (self.roadInspectVC && [[self.navigationController visibleViewController] isEqual:self.roadInspectVC]) {
        if (![self.closeID isEmpty]) {
            [self.roadInspectVC createRecodeByDynamicInfo:[[RoadWayClosed roadWayCloseInfoForID:self.closeID] objectAtIndex:0]];
            [self setCloseID:nil];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (closeInfo.isuploaded.boolValue) {
        ...
        [self.uiBtnPromulgate setEnabled:NO];
        [self.uiBtnPromulgate setAlpha:0.0];
...
- (IBAction)btnSave:(UIButton *)sender {
    ...
    if ([self.textTitle.text isEmpty]) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^(void){
            ...
            closeInfo.remark         = self.textremark.text;
            closeInfo.org_id=[[OrgInfo orgInfoForSelected] valueForKey:@"myid"];
            [[AppDelegate App] saveContext];
...
- (IBAction)btnPhoto:(id)sender {
    if(self.closeID == nil || [self.closeID isEmpty]){
    ...
    [next setValue:self.closeID forKey:@"constructionId"];
