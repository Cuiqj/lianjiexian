...
@property (nonatomic,retain) UISwitch * switchremark;
//照片路径
...
- (void)viewDidLoad
{
    ...
    NSLog(@"%f----%f",self.remark.frame.origin.x,self.remark.frame.origin.y);
    if ([self.attachmentWho isEqualToString:@"构造物检查"]) {
        self.switchremark = [[UISwitch alloc]initWithFrame:CGRectMake(355, 128, 50, 15)];
        UILabel * label = [[UILabel alloc] initWithFrame:CGRectMake(350, 165, 80, 20)];
        [self.switchremark addTarget:self action:@selector(switchchange:) forControlEvents:UIControlEventValueChanged];
        label.text = @"有无异常";
        [self.view addSubview:label];
        [self.view addSubview:self.switchremark];
    }
    if([self.photos count]>0){
            CasePhoto * casePhotoInfo=[self.photos objectAtIndex:0];
            if ([[[NSUserDefaults standardUserDefaults]objectForKey:casePhotoInfo.myid] length]) {
                self.switchremark.on = YES;
            }
    }
...
- (void)switchchange:(UISwitch *)swtichremark{
    if (swtichremark.on) {
         self.attachmentremark = self.remark.text;
    }
}
...
- (void)viewWillDisappear:(BOOL)animated{
    if (self.attachmentid && self.attachmentremark) {
        [[NSUserDefaults standardUserDefaults] setObject:self.attachmentremark forKey:self.attachmentid];
    }
    self.attachmentid = nil;
    self.attachmentremark = nil;
    self.attachmentWho = nil;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([[[NSUserDefaults standardUserDefaults]objectForKey:self.casePhotoId] length]) {
        self.switchremark.on = YES;
    }else if ([self.casePhotoId isEqualToString:self.attachmentid]) {
        self.switchremark.on = YES;
    }else{
        self.switchremark.on = NO;
    }
...
- (void)btnSavemessage{
    if(self.casePhotoId != nil && ![self.casePhotoId isEmpty]){
        ...
        if (self.switchremark.on) {
            self.attachmentremark = self.remark.text;
        }
        self.attachmentid = self.casePhotoId;
    }else{
        ...
        self.attachmentid = newPhoto.myid;
    }
    [[AppDelegate App] saveContext];
    if (!self.switchremark.on) {
        self.attachmentremark = nil;
        [[NSUserDefaults standardUserDefaults]setObject:@"" forKey:self.attachmentid];
        self.attachmentid = nil;
    }
    self.photos = [[CasePhoto casePhotos:self.constructionId]mutableCopy];
