...
- (UIImage*) imageBlackToTransparent:(UIImage*) image{// 分配内存
...
-(void)imageRepresentation {
    ...
    if (img.size.width > 0) {
         [self.delegate getSignatureImg:[self scaleToSize:image]];
    }
}
//压缩图片,最长边为128
- (UIImage *)scaleToSize:(UIImage *)img {
    ...
    //判断图片宽度
    if(imageWidth >= 400){
        rect =CGRectMake(0,0, 400, img.size.height*400/img.size.width);
    }else if (img.size.height >= 200){
        rect =CGRectMake(0,0, img.size.width*200/img.size.height, 200);
    }else{
...
//    UIImage *lastImage = [self addText:img text:self.showMessage];
    UIImage *lastImage = [self addText:img text:@""];
    [self setNeedsDisplay];
    return lastImage;
}
//签名完成，给签名照添加新的水印
