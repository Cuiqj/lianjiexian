...
- (void)viewDidLoad{
    ...
    //  self.roadModelBoard.backgroundColor=[UIColor redColor];
    [self.roadModelBoard setUserInteractionEnabled:YES];
    ...
    // self.paintBoard.backgroundColor=[UIColor colorWithWhite:100 alpha:0.5];
...
- (IBAction)btnSave:(UIBarButtonItem *)sender {
    if (![self.caseID isEmpty]) {
        ...
        //dispatch_async(myqueue, ^(void){
            ...
            if (casemap == nil) {
                ...
                if (caseProveInfo && caseProveInfo.event_desc != nil) {
                    ...
                    //casemap.remark = caseProveInfo.caseMapRemark;
                } else {
        ...
        NSLog(@"subviews=%lu",(unsigned long)[self.paintBoard.subviews count]);
        NSMutableArray *MoveableImages=[[NSMutableArray alloc]init];
        for (MoveableImage *moveImage in self.paintBoard.subviews)
            [ MoveableImages addObject:moveImage];
       [ MoveableImages sortedArrayWithOptions:NSSortStable usingComparator:^NSComparisonResult(id obj1,id obj2){
           MoveableImage *id1=(MoveableImage*)obj1;
           MoveableImage *id2=(MoveableImage*)obj2;
           return id1.iconModelID>id2.iconModelID;
        }];
        // for (MoveableImage *moveImage in self.paintBoard.subviews)
        for (MoveableImage *moveImage in MoveableImages) {
                if ([moveImage isMemberOfClass:[MoveableImage class]]) {
                    NSString *nameString = moveImage.iconModelID;
                    IconModels *theIconModel=[IconModels iconModelforID:moveImage.iconModelID];
                    NSString *fontSizeString;
                    if (moveImage.fontSize > 0) {
                        //fontSizeString = [NSString stringWithFormat:@"%d",(int)moveImage.fontSize];
                        fontSizeString = [NSString stringWithFormat:@"%f", moveImage.fontSize];
                    } else {
                        fontSizeString = nil;
                    }
                    ...
                    NSString *heightString = [NSString stringWithFormat:@"%f", (moveImage.imageSize.height/zoom)];
                    NSString *widthString = [NSString stringWithFormat:@"%f", (moveImage.imageSize.width/zoom)];
                    NSString *topString = [NSString stringWithFormat:@"%f", ((moveImage.center.y-moveImage.imageSize.height/2-RoadModelOffSet)/zoom)];
                    NSString *leftString = [NSString stringWithFormat:@"%f", ((moveImage.center.x-moveImage.imageSize.width/2)/zoom)];
                    ...
                    if (moveImage.isText) {
                        ...
                        DDXMLElement *typeAttri = [DDXMLNode attributeWithName:@"Type" stringValue:theIconModel.icontype];
                        DDXMLElement *nameAttri = [DDXMLNode attributeWithName:@"Name" stringValue:theIconModel.iconname];
            ...
            for (DWStroke *stroke in self.paintBoard.dwStrokes) {
                NSString *typeString = [[NSString alloc] initWithFormat:@"%ld", (long)stroke.pathType];
    ...
    //更新草图预览界面
    if (![self.caseID isEmpty]) {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"RELOAD_PAINT_BRIEF" object:nil];
    }
}
...
-(void)inputString:(NSString *)text fontSize:(CGFloat)fontSize inRect:(CGRect)rect{
    ...
    [moveableImage setUserInteractionEnabled:YES];
    [moveableImage setMultipleTouchEnabled:YES];
        ...
        if (caseMap.map_item && ![caseMap.map_item isEmpty]) {
            ...
            if (backGroundItemsArray.count > 0 || backGroundTextsArray.count > 0) {
                ...
                for (DDXMLElement *text in backGroundTextsArray) {
                    ...
                    CGFloat textTop = [text attributeForName:@"Top"].stringValue.floatValue * zoom+RoadModelOffSet;
            ...
            for (DDXMLElement *icon in iconsArray) {
                ...
                CGFloat iconTop = [icon attributeForName:@"Top"].stringValue.floatValue *zoom+RoadModelOffSet;
