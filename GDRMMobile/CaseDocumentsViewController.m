...
- (void)viewDidLoad
{
    ...
    if (self.docPrinterState==kPDFView) {
        ...
        NSString * sss=[self.TableMapping objectForKey:self.fileName];
        self.docPrinter.caseID=self.caseID;
        //通知书和清单可以套打  其他的屏蔽套打
        if ([[self.TableMapping objectForKey:self.fileName] isEqualToString:@"CaseCountTable"] ||
            [[self.TableMapping objectForKey:self.fileName] isEqualToString:@"AtonementNoticeTable"]){
            [self.uiButtonPrintFull removeFromSuperview];
        }else{
            [self.uiButtonPrintForm removeFromSuperview];
        }
...
-(IBAction)btnPrintFormedPDF:(id)sender{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"UPDATE_CASE_FILE" object:nil];
}
