...
- (void)viewDidLoad
{
    ...
    [self.labelRoadType setHidden:YES];
    [self.LabelRoadType_text setHidden:YES];
    [self.LabelCar setHidden:YES];
    [self.labelCitizen setHidden:YES];
}
...
- (void)loadPageInfo{
    ...
    if (caseMap) {
        ...
        if(caseInfo.sfz_id.integerValue>0)
            ...
            locality= [[NSString alloc] initWithFormat:@"%@高速%@K%02d+%003dm%@",[[AppDelegate App].projectDictionary objectForKey:@"cityname"] ,caseInfo.side,caseInfo.station_start.integerValue/1000,caseInfo.station_start.integerValue%1000,caseInfo.place];
        ...
        if (caseProveInfo) {
            ...
            self.LabelCase_long_desc.text = caseProveInfo.case_long_desc;
        }
