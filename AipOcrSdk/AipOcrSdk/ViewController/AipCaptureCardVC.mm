...
- (void)viewDidLoad {
    [super viewDidLoad];
    UIAlertView *alert =[[UIAlertView alloc]initWithTitle:@"title" message:@"message" delegate:self cancelButtonTitle:@"hao" otherButtonTitles:nil, nil] ;
    alert.tag=666;
    //[alert show];
    ...
    [self setupCmmotionManager];[[UIDevice currentDevice] beginGeneratingDeviceOrientationNotifications];
...
-(void)willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration{
    AVCaptureConnection *captureConnection=[self.cameraController.previewLayer connection];
    captureConnection.videoOrientation=(AVCaptureVideoOrientation)toInterfaceOrientation;
}

//-(void)didRotateFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation{
//    self.cameraController.previewLayer.frame=self.view.bounds;
//}
#pragma mark - SetUp
//判断设备方向
-(void)deviceOrientationDidChange:(NSObject*)sender{
    ...
    self.curDeviceOrientation=device.orientation;
    ...
    if([self.cmmotionManager isDeviceMotionAvailable]) {
        [self.cmmotionManager startAccelerometerUpdatesToQueue:[NSOperationQueue currentQueue] withHandler:^(CMAccelerometerData * _Nullable accelerometerData, NSError * _Nullable error) {
            ...
            if (accelerometerData.acceleration.x >= 0.75) {//home button left
                ...
                [[wself.cameraController.previewLayer
                  connection]
...
self.bankCardView.layer.borderColor = [UIColor redColor].CGColor;
self.bankCardView.layer.borderWidth = 1.0;
self.bankCardView.translatesAutoresizingMaskIntoConstraints = NO;
self.bankCardView.layer.borderColor = [UIColor greenColor].CGColor;
self.bankCardView.layer.borderWidth = 1.0;
self.bankCardView.layer.cornerRadius = bankCardViewCornerRadius *[self.class speScale];
self.cutImageView.layer.borderColor=[UIColor yellowColor].CGColor;
self.previewView.transform=[self getOriginFangXiang];
}
-(CGAffineTransform)getOriginFangXiang{
...
- (void)orientationChanged{
    ...
    }else if(self.curDeviceOrientation == UIDeviceOrientationPortraitUpsideDown){
        return;
        transform = CGAffineTransformMakeRotation(-M_PI);
        
        self.tipsLabelCenterX.constant = -140*[self.class speScale];
        self.tipsLabelCenterY.constant = -80*[self.class speScale];
        self.imageDeviceOrientation = UIDeviceOrientationPortraitUpsideDown;
    }else {
        return;
            ...
            if (isBack) {
                ...
                [[AipOcrService shardService] detectIdCardBackFromImage:self.finalImage withOptions:nil successHandler:^(id result) {
                   ...
                   // [wself ocrOnIdCardSuccessful:result];
                    if ([wself.delegate respondsToSelector:@selector(ocrOnIdCardSuccessful:)]) {
                        [wself.delegate ocrOnIdCardSuccessful:result];
                    }
                } failHandler:^(NSError *err) {
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    if (self.displaylinkisOn) {
        ...
        @autoreleasepool {
            ...
            image =[UIImage image:image rotation:UIImageOrientationLeft]; //[UIImage imageWithCGImage:image.CGImage scale:2.0 orientation:UIImageOrientationRight];
